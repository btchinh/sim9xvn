...
class sim extends CI_Controller {
	...
	public function import()
	{
		...
		###SPEED 2
		//var_dump($_POST['sim_number']);die;
		$sim_original = array();
		$price_buy = array();
		if(isset($_POST['sim_number']) && $_POST['sim_number'] != '' && isset($_POST['sim_price']) && $_POST['sim_price'] != ''){
			$arraySIMImport = array();
			
			$sim_original = preg_split('/[\s]+/',$_POST['sim_number']);
			$price_buy = preg_split('/[\s]+/',$_POST['sim_price']);
			$i = 0;

			//var_dump(preg_split('/[\s]+/',$_POST['sim_number'])[0]);die;
			while ($i <$limitrecord) {
				$arraySIMImport[] = array(
			    	'sim_original' => $sim_original[$i],
			       	'price_buy' => $price_buy[$i],					
			       	);		
				$i++;
			}
			//var_dump($arraySIMImport[0]);die;

			if(!$this->sim_model->insert('sim',$arraySIMImport)){
